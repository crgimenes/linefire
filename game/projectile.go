...
func (g *Game) stepEnemyShots() {
	...
	for i := range shots {
		...
		if hitRock {
			if p.aoe > 0 {
				g.shipBlast(rx, ry, p.aoe, p.hullDmg, p.blastSource()) // a missile into rock still detonates
				continue
			}
		...
		if hit := g.bulletHitsFoe(p.px, p.py, nx, ny, p.faction); hit >= 0 {
			if p.aoe > 0 {
				g.shipBlast(nx, ny, p.aoe, p.hullDmg, p.blastSource()) // an area weapon: the blast decides who it hurts
				continue
			}
